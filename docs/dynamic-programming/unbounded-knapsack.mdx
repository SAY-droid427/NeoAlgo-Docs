...
We start from the nth item whose weight is 10 which is less than W.
 So we can either choose it or not depending on what will maximise the value of the knapsack.
...
We start from the nth item whose weight is equal to the knapsack W, so we may either include it or exclude it. 
If we include it the value of the knapsack will be `15`, but if we exclude it the value of the knapsack will be `10`. 
So the maximum value of the knapsack will be `15`.
```

## Algorithm

- Create a table (let it be called dp here). 
  In this table `dp[i][j]` will represent the value of the knapsack if there are `i` items and the weight of the knapsack is `j`.
